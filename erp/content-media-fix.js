...
(function(){
  ...
  window.openContentWithMedia=async function(id=null){
    ...
    document.querySelector('#modalBox').innerHTML=`<h2>${id?'Edit':'Add'} Content</h2><div class="form">
      ...
      <label>Content Image / Video<input id="cm_media" type="file" accept="image/*,video/*"><small>Upload an image or video. Maximum 500 MB.</small></label>
    ...
    document.querySelector('#cm_media').addEventListener('change',e=>{const f=e.target.files?.[0];if(!f)return;const url=URL.createObjectURL(f);document.querySelector('#cm_media_preview').innerHTML=f.type.startsWith('video/')?`<video src="${url}" controls class="media-preview"></video>`:`<img src="${url}" class="media-preview" alt="Selected media">`});
  };
  ...
  window.saveContentWithMedia=async function(id){
    ...
    if(!title)return toast('Please enter a content title');if(!client)return toast('Please select a client');save.disabled=true;save.textContent='Uploading / saving...';
    try{
      ...
      if(file){
        ...
        const ext=(file.name.split('.').pop()||'bin').toLowerCase(),path=`${user.id}/${Date.now()}-${crypto.randomUUID()}.${ext}`;
  ...
  const originalOpen=window.openEntity,originalContentTable=window.contentTable;
  ...
  window.contentTable=function(items){
    const html=originalContentTable(items);
    const rows=items.map(x=>{const media=x.media_url?(x.media_type?.startsWith('video/')?`<video src="${esc(x.media_url)}" class="table-media" muted controls></video>`:`<img src="${esc(x.media_url)}" class="table-media" alt="Content">`):`<span class="no-media">No media</span>`;return media});
    let i=0;return html.replace('<tr><th>Schedule</th>','<tr><th>Media</th><th>Schedule</th>').replace(/<tr>(?=<td>)/g,()=>`<tr><td>${rows[i++]||'<span class="no-media">No media</span>'}</td>`);
  };
